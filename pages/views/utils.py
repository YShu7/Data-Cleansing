...
def get_controversial_voting_data(group=None, search_term=None):
    if group:
        voting_data = VotingData.objects.filter(is_contro=True, group=group, is_active=True)
    if search_term:
        voting_data = VotingData.objects.filter(is_contro=True, title__icontains=search_term, is_active=True)
    if group is None and search_term is None:
        voting_data = VotingData.objects.filter(is_contro=True, is_active=True)
